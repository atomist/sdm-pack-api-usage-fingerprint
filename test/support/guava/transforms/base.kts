...
class ExecuteTransformCommand(val transformers: List<(CompilationUnit, File) -> Unit>) : CliktCommand() {
    ...
    override fun run() {
        val testFolderExists = File(path + File.separator + testSrcFolder).exists();
        val srcFolders: List<String> = if(testFolderExists) { listOf(srcFolder, testSrcFolder) } else { listOf(srcFolder) }
